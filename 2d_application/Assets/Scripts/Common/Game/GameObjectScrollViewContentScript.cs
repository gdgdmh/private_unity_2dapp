...
public class GameObjectScrollViewContentScript : MonoBehaviour {

	// Use this for initialization
	void Start () {
        int size = scroll_view_objects_.Length;
        for (int i = 0; i < size; ++i) {
            GameObject obj = CreateScrollViewObject(i);
        }
    }
    ...
    GameObject CreateScrollViewObject(int index) {
		// index over
		if (index >= scroll_view_objects_.Length) {
			return null;
        }
		GameObject obj = Instantiate(scroll_view_objects_[index]);
        // content_rect_transform_に関連付け
        obj.transform.SetParent (content_rect_transform_.transform, false);
		return  obj;
    }

    // 親
    public RectTransform content_rect_transform_;
    // 複製元のプレハブ
    public GameObject[] scroll_view_objects_;
